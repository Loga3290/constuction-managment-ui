...
export class HolisticViewComponent {
  ...
  generateTable() {
    this.foremanDetails = [];
    this.httpservice.getEntries(this.selectedSites, [this.selectedForeman], this.days[0], this.days[6])
      .subscribe(
        res => {
          this.entries = res;
          ...
          this.foremanDetails.forEach(foremanDtl => {
            ...
            this.getAdvances().subscribe((advance: number) => {
              foremanDtl.total = total;
              foremanDtl.advance = advance;
              foremanDtl.finalBalance = total - advance;
            })
  ...
  public getAdvances() {
    return new Observable((observer) => {
      this.httpservice.getForemanAdvancesForForeman(this.selectedForeman.id)
      .subscribe(
        res => {
          let advance = 0;
          this.foremanAdvances = res;
          this.foremanAdvances.forEach(foremanAdvance => {
            advance += foremanAdvance.amount
          })
          observer.next(advance)
          observer.complete();  
        },
        err => console.log(err)
      )
